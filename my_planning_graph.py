...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            precond_pos_satisfied = set(action.precond_pos).issubset(s_node.symbol
                for s_node in s_level if s_node.is_pos)
            precond_neg_satisfied = set(action.precond_neg).issubset(s_node.symbol
                for s_node in s_level if not s_node.is_pos)

            if precond_pos_satisfied and precond_neg_satisfied:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        return all(s1_parent.is_mutex(s2_parent)
            for s1_parent in node_s1.parents for s2_parent in node_s2.parents)
